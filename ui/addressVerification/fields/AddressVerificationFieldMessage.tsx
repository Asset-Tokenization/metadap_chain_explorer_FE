...
const AddressVerificationFieldMessage = ({ formState, control }: Props) => {
    ...
    ({ field }: { field: ControllerRenderProps<Fields, 'message'> }) => {
      ...
      return (
        <FormControl variant="floating" id={field.name} isRequired size="md" backgroundColor={backgroundColor}>
          <Textarea {...field} required isInvalid={Boolean(error)} isDisabled autoComplete="off" maxH={{ base: '140px', lg: '80px' }} />
  ...
  return <Controller defaultValue="some value" name="message" control={control} render={renderControl} rules={{ required: true }} />;
};
