...
const AddressVerificationFieldAddress = ({ formState, control }: Props) => {
    ...
    ({ field }: { field: ControllerRenderProps<Fields, 'address'> }) => {
      ...
      return (
        <FormControl variant="floating" id={field.name} isRequired size="md" backgroundColor={backgroundColor} mt={8}>
          <Input {...field} required isInvalid={Boolean(error)} maxLength={ADDRESS_LENGTH} isDisabled={formState.isSubmitting} autoComplete="off" />
  ...
  return <Controller name="address" control={control} render={renderControl} rules={{ required: true, pattern: ADDRESS_REGEXP }} />;
};
