...
const AddressVerificationModal = ({ defaultAddress, isOpen, onClose, onSubmit, onAddTokenInfoClick, onShowListClick, pageType }: Props) => {
  ...
  React.useEffect(() => {
    isOpen && mixpanel.logEvent(mixpanel.EventTypes.VERIFY_ADDRESS, { Action: 'Form opened', 'Page type': pageType });
  }, [isOpen, pageType]);
  ...
  const handleGoToSecondStep = React.useCallback(
    (firstStepResult: typeof data) => {
      ...
      mixpanel.logEvent(mixpanel.EventTypes.VERIFY_ADDRESS, { Action: 'Address entered', 'Page type': pageType });
    },
    [pageType],
  );
  ...
  const handleGoToThirdStep = React.useCallback(
    (address: VerifiedAddress, signMethod: 'wallet' | 'manual') => {
      ...
      mixpanel.logEvent(mixpanel.EventTypes.VERIFY_ADDRESS, { Action: 'Sign ownership', 'Page type': pageType, 'Sign method': signMethod });
    },
    [onSubmit, pageType],
  );
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} size={{ base: 'full', lg: 'md' }}>
      ...
      <ModalContent>
        ...
        <ModalCloseButton />
        <ModalBody mb={0}>
          <Web3ModalProvider fallback={step?.fallback || step.content}>{step.content}</Web3ModalProvider>
        </ModalBody>
      </ModalContent>
    </Modal>
  );
