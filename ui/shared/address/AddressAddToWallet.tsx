...
const AddressAddToWallet = ({ className, token, isLoading, variant = 'icon', iconSize = 6 }: Props) => {
    ...
    try {
      ...
      if (wasAdded) {
        toast({
          position: 'top-right',
          title: 'Success',
          description: 'Successfully added certificate to your wallet',
          status: 'success',
          variant: 'subtle',
          isClosable: true,
        });
  ...
  if (variant === 'button') {
    return (
      <Tooltip label={`Add certificate to ${WALLETS_INFO[wallet].name}`}>
        <IconButton
          className={className}
          aria-label="Add certificate to wallet"
          variant="outline"
          size="sm"
          px="6px"
          onClick={handleClick}
