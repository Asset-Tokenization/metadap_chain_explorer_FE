...
const CodeEditorFileExplorer = ({ data, onFileSelect, selectedFile, mainFile, isActive, setActionBarRenderer }: Props) => {
  ...
  const renderActionBar = React.useCallback(() => {
    return <CoderEditorCollapseButton onClick={handleCollapseButtonClick} label="Collapse folders" />;
  ...
  return (
    <Box>
      <CodeEditorFileTree key={key} tree={tree} onItemClick={handleFileClick} isCollapsed={key > 0} selectedFile={selectedFile} mainFile={mainFile} />
    </Box>
  );
};
