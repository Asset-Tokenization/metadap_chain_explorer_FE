...
export default function PublicTagFormAction({ control, index, fieldsLength, error, onAddFieldClick, onRemoveFieldClick, size }: Props) {
  const renderAddressInput = useCallback(
    ({ field }: { field: ControllerRenderProps<Inputs, `addresses.${number}.address`> }) => {
      return <AddressInput<Inputs, `addresses.${number}.address`> field={field} error={error} size={size} placeholder="Smart contract / Address (0x...)" />;
    },
    [error, size],
  );
  ...
  return (
    <Flex flexDir="column" rowGap={5} alignItems="flex-end">
      ...
      />
      <Flex columnGap={5} position={{ base: 'static', lg: 'absolute' }} left={{ base: 'auto', lg: 'calc(100% + 20px)' }} h="100%" alignItems="center">
        {fieldsLength > 1 && (
          <IconButton
            aria-label="delete"
            variant="outline"
        ...
        {index === fieldsLength - 1 && fieldsLength < MAX_INPUTS_NUM && (
          <IconButton aria-label="add" variant="outline" w="30px" h="30px" onClick={onAddFieldClick} icon={<Icon as={PlusIcon} w="20px" h="20px" />} />
        )}
      </Flex>
    </Flex>
  );
