...
const TokenDetails = ({ tokenQuery }: Props) => {
  ...
  if (tokenQuery.isError) {
    throw Error('Token fetch error', {
      cause: tokenQuery.error as unknown as Error,
    });
  }

  const { exchange_rate: exchangeRate, total_supply: totalSupply, circulating_market_cap: marketCap, decimals, symbol, type } = tokenQuery.data || {};
  ...
  return (
    <Grid columnGap={8} rowGap={{ base: 1, lg: 3 }} templateColumns={{ base: 'minmax(0, 1fr)', lg: 'auto minmax(0, 1fr)' }} overflow="hidden">
      {exchangeRate && (
        <DetailsInfoItem title="Price" hint="Price per certificate on the exchanges" alignSelf="center" isLoading={tokenQuery.isPlaceholderData}>
      ...
      {marketCap && (
        <DetailsInfoItem title="Fully diluted market cap" hint="Total supply * Price" alignSelf="center" isLoading={tokenQuery.isPlaceholderData}>
          <Skeleton isLoaded={!tokenQuery.isPlaceholderData} display="inline-block">
      ...
      <DetailsInfoItem
        ...
        hint="The total amount of certificates issued"
        alignSelf="center"
      ...
      <DetailsInfoItem title="Holders" hint="Number of accounts holding the certificate" alignSelf="center" isLoading={tokenQuery.isPlaceholderData}>
        <Skeleton isLoaded={!tokenCountersQuery.isPlaceholderData}>{countersItem('token_holders_count')}</Skeleton>
      </DetailsInfoItem>
      <DetailsInfoItem title="Transfers" hint="Number of transfer for the certificate" alignSelf="center" isLoading={tokenQuery.isPlaceholderData}>
        <Skeleton isLoaded={!tokenCountersQuery.isPlaceholderData}>{countersItem('transfers_count')}</Skeleton>
      </DetailsInfoItem>
      {decimals && (
        <DetailsInfoItem
          title="Decimals"
          hint="Number of digits that come after the decimal place when displaying certificate value"
          alignSelf="center"
          isLoading={tokenQuery.isPlaceholderData}
        >
