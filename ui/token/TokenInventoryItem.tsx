...
const NFTItem = ({ item, isLoading }: Props) => {
  ...
  const mediaElement = <NftMedia mb="18px" url={item.animation_url || item.image_url} isLoading={isLoading} />;
    ...
    >
        ...
        <Flex mb={2} ml={1}>
          ...
          <TruncatedTextTooltip label={item.id}>
            <Skeleton isLoaded={!isLoading} overflow="hidden">
              <LinkInternal overflow="hidden" textOverflow="ellipsis" whiteSpace="nowrap" display="block" isLoading={isLoading} href={url}>
                {item.id}
              </LinkInternal>
            </Skeleton>
      ...
      {item.owner && (
        <Flex mb={2} ml={1}>
          ...
          <AddressEntity address={item.owner} isLoading={isLoading} truncation="constant" noCopy noIcon={isMobile} />
        </Flex>
      )}
    </Box>
  );
