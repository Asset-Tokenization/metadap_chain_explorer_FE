...
const TokenTransfer = ({ transfersQuery, tokenId, token }: Props) => {
  ...
  const content = data?.items ? (
    <>
      ...
      <Box display={{ base: 'block', lg: 'none' }}>
        {pagination.page === 1 && (
          <SocketNewItemsNotice.Mobile url={window.location.href} num={newItemsCount} alert={socketAlert} type="token_transfer" isLoading={isPlaceholderData} />
        )}
  ...
  return <DataListDisplay isError={isError} items={data?.items} emptyText="There are no certificate transfers." content={content} actionBar={actionBar} />;
};
