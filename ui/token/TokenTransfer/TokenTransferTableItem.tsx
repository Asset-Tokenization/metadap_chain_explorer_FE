...
const TokenTransferTableItem = ({ token, total, tx_hash: txHash, from, to, method, timestamp, tokenId, isLoading }: Props) => {
    ...
    <Tr alignItems="top">
      <Td>
        <Grid alignItems="center" gridTemplateColumns="auto 130px" width="fit-content" py="7px">
          <TxEntity hash={txHash} isLoading={isLoading} fontWeight={600} noIcon />
          {timestamp && (
      ...
      <Td>
        <AddressEntityWithTokenFilter address={from} isLoading={isLoading} truncation="constant" tokenHash={token.address} my="5px" />
      </Td>
      <Td px={0}>
        <Box my="3px">
      ...
      <Td>
        <AddressEntityWithTokenFilter address={to} isLoading={isLoading} truncation="constant" tokenHash={token.address} my="5px" />
      </Td>
