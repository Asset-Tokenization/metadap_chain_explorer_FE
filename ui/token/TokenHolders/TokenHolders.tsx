...
const TokenHoldersContent = ({ holdersQuery, token }: Props) => {
    ...
    items && token ? (
      <>
        <Box display={{ base: 'none', lg: 'block' }}>
          <TokenHoldersTable data={items} token={token} top={holdersQuery.pagination.isVisible ? 80 : 0} isLoading={holdersQuery.isPlaceholderData} />
        </Box>
  ...
  return (
    <DataListDisplay
      ...
      emptyText="There are no holders for this certificate."
      content={content}
      actionBar={actionBar}
    />
  );
