  ...
  return (
    <Flex
      ...
      <Box
        ...
        pt={{ base: 4, lg: 6 }}>
        <Text fontSize="xs">
          DAP Creadit (DAP) là native token của MetaDAP Enterprise Blockchain, DAP được sử dụng như một credit service, nhà phát triển cần đăng ký (subscription) các gói DAP Credit để được cấp quyền ghi dữ liệu vào blockchain, DAP Credit sẽ được khấu trừ dần trong quá trình sử dụng. DAP hoàn toàn chỉ được sử dụng với mục đích quản trị, DAP không phải là phương tiện thanh toán, MetaDAP Enterprise Blockchain chỉ sử dụng duy nhất một phương tiện thanh toán là tiền pháp định (Fiat). DAP có thể được tặng để khuyến khích và tạo điều kiện thuận lợi cho cộng đồng tiếp cận và dùng thử MetaDAP Enterprise Blockchain, DAP hoàn toàn không được sử dụng với mục đích gọi vốn.
        </Text>
        <Text mt={ 6 } fontSize="xs">
          DAP Credit (DAP) is the native token of the MetaDAP Enterprise
          Blockchain. DAP is used as a credit service, where developers must
          subscribe to DAP Credit packages to gain permission to write data to
          the blockchain. The DAP Credit balance will gradually be deducted
          during usage. DAP is strictly for governance purposes and is not a
          means of payment. The MetaDAP Enterprise Blockchain exclusively uses
          fiat currency as the sole payment method. DAP can be granted as an
          incentive to encourage and facilitate community access and trial usage
          of the MetaDAP Enterprise Blockchain. However, DAP is strictly
          prohibited from being used for fundraising purposes.
        </Text>
      </Box>
      <Flex
      ...
      >
        <Box flexGrow="1" mb={{ base: 8, lg: 0 }}>
          ...
          <Text mt={ 3 } maxW={{ base: 'unset', lg: '470px' }} fontSize="xs">
            MetaDAP explorer is a tool for inspecting and analyzing EVM based
          ...
          <Box minW="160px" w={ config.UI.footer.links ? '160px' : '100%' }>
            ...
            <Grid
              ...
              gridTemplateColumns={
                config.UI.footer.links ?
                  '160px' :
                  { base: 'repeat(auto-fill, 160px)', lg: 'repeat(4, 160px)' }
              }
              gridTemplateRows={{
                base: 'auto',
