  ...
  return (
    ...
    >
      <Box px={{ base: 4, lg: 9 }} pt={{ base: 4, lg: 6 }}>
        <Text fontSize="xs">
          DAP Creadit (DAP) được sử dụng như một credit service, nhà phát triển
          cần đăng ký (subscription) các gói DAP Credit để được cấp quyền ghi dữ
          liệu vào blockchain, DAP Credit sẽ được khấu trừ dần trong quá trình
          sử dụng. DAP hoàn toàn chỉ được sử dụng với mục đích quản trị, DAP
          không phải là phương tiện thanh toán, MetaDAP Enterprise Blockchain
          chỉ sử dụng duy nhất một phương tiện thanh toán là tiền pháp định
          (Fiat). DAP có thể được tặng để khuyến khích và tạo điều kiện thuận
          lợi cho cộng đồng tiếp cận và dùng thử MetaDAP Enterprise Blockchain,
          DAP hoàn toàn không được sử dụng với mục đích gọi vốn.
        </Text>
        <Text mt={6} fontSize="xs">
          DAP Credit (DAP) is used as a credit service, where developers must
