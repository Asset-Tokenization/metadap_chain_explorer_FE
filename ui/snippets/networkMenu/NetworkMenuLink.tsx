...
const NetworkMenuLink = ({ title, icon, isActive, isMobile, url, invertIconInDarkMode }: Props) => {
  ...
  const iconEl = icon ? (
    ...
    <Icon as={placeholderIcon} boxSize="30px" color={colors.iconPlaceholder.default} />
  );
  ...
  return (
    <Box as="li" listStyleType="none">
      ...
      >
        {iconEl}
        <Text marginLeft={3} fontWeight="500" color="inherit" fontSize={isMobile ? 'sm' : 'md'} lineHeight={isMobile ? '20px' : '24px'}>
          {title}
        </Text>
