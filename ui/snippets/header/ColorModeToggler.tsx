...
import type { SystemStyleObject, ThemingProps, HTMLChakraProps } from '@chakra-ui/system';
...
const ColorModeToggler = forwardRef<ColorModeTogglerProps, 'input'>((props, ref) => {
  ...
  const { state, getInputProps, getCheckboxProps, getRootProps } = useCheckbox({ ...ownProps, isChecked: colorMode === 'light' });
  ...
  return (
    <chakra.label {...getRootProps({ onChange: toggleColorMode })} display="inline-block" position="relative" verticalAlign="middle" lineHeight={0}>
      <chakra.input
        {...getInputProps({}, ref)}
        border="none"
        height="1px"
        ...
        position="absolute"
      />
      <chakra.div {...getCheckboxProps()} __css={trackStyles} aria-label="Toggle color mode">
        <Icon boxSize={4} margin={2} zIndex="docked" as={moonIcon} color={useColorModeValue('blue.300', 'blackAlpha.900')} {...transitionProps} />
        <chakra.div data-checked={dataAttr(state.isChecked)} data-hover={dataAttr(state.isHovered)} __css={thumbStyles} />
        <Icon boxSize={5} margin={1.5} zIndex="docked" as={sunIcon} color={useColorModeValue('blackAlpha.900', 'blue.300')} {...transitionProps} />
      </chakra.div>
    </chakra.label>
  );
});
