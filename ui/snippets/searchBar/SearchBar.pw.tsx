...
test('search by token name  +@mobile +@dark-mode', async ({ mount, page }) => {
  ...
  await page.route(API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify([searchMock.token1, searchMock.token2]),
    }),
  );
...
test('search by contract name  +@mobile +@dark-mode', async ({ mount, page }) => {
  ...
  await page.route(API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify([searchMock.contract1]),
    }),
  );
...
test('search by name homepage +@dark-mode', async ({ mount, page }) => {
  ...
  await page.route(API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify([searchMock.token1, searchMock.token2, searchMock.contract1]),
    }),
  );
...
test('search by tag  +@mobile +@dark-mode', async ({ mount, page }) => {
  ...
  await page.route(API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify([searchMock.label1]),
    }),
  );
...
test('search by address hash +@mobile', async ({ mount, page }) => {
  ...
  await page.route(API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify([searchMock.address1]),
    }),
  );
...
test('search by block number +@mobile', async ({ mount, page }) => {
  ...
  await page.route(API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify([searchMock.block1, searchMock.block2]),
    }),
  );
...
test('search by block hash +@mobile', async ({ mount, page }) => {
  ...
  await page.route(API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify([searchMock.block1]),
    }),
  );
...
test('search by tx hash +@mobile', async ({ mount, page }) => {
  ...
  await page.route(API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify([searchMock.tx1]),
    }),
  );
...
test('search with view all link', async ({ mount, page }) => {
  ...
  await page.route(API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify([searchMock.token1, searchMock.token2, searchMock.contract1, ...Array(47).fill(searchMock.contract1)]),
    }),
  );
...
test('recent keywords suggest +@mobile', async ({ mount, page }) => {
  ...
  await page.evaluate(() =>
    window.localStorage.setItem(
      ...
      '["10x2d311959270e0bbdc1fc7bc6dbd8ad645c4dd8d6aa32f5f89d54629a924f112b","0x1d311959270e0bbdc1fc7bc6dbd8ad645c4dd8d6aa32f5f89d54629a924f112b","usd","bob"]',
    ),
  );
...
base.describe('with apps', () => {
  ...
  test('default view +@mobile', async ({ mount, page }) => {
    ...
    await page.route(API_URL, (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify([searchMock.token1]),
      }),
    );
