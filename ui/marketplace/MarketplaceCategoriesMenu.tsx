...
const MarketplaceCategoriesMenu = ({ selectedCategoryId, onSelect, categories, isLoading }: Props) => {
  const options = React.useMemo(() => [MarketplaceCategory.FAVORITES, MarketplaceCategory.ALL, ...categories], [categories]);

  if (isLoading) {
    return <Skeleton h="40px" w={{ base: '100%', sm: '120px' }} borderRadius="base" mb={{ base: 2, sm: 0 }} mr={{ base: 0, sm: 2 }} />;
  }

  return (
    <Menu>
      <MenuButton as={Button} mb={{ base: 2, sm: 0 }} mr={{ base: 0, sm: 2 }} size="md" variant="outline" colorScheme="gray" flexShrink={0}>
      ...
      <MenuList zIndex={3}>
        {options.map((category: string) => (
          <MarketplaceCategoriesMenuItem key={category} id={category} onClick={onSelect} />
        ))}
      </MenuList>
    </Menu>
  );
