...
const MetadataItemArray = ({ name, value, level }: Props) => {
  return (
    <MetadataAccordionItem flexDir={{ lg: 'column' }} alignItems="stretch" pl={{ base: 0, lg: 0 }} py={0}>
      <AccordionButton
        px={0}
        py={2}
      ...
      <AccordionPanel p={0} ml={{ base: 6, lg: level === 0 ? '126px' : 6 }}>
        {value.map((item, index) => {
                    ...
                    return Object.entries(item).map(([name, value], index) => {
                      return (
                        <Flex key={index} columnGap={3}>
                          ...
                          <MetadataItemPrimitive value={typeof value === 'object' ? JSON.stringify(value, undefined, 2) : value} isItem={false} level={level} />
                        </Flex>
                      );
                    });
                  }
          ...
          return (
            <Flex key={index} py={2} _notFirst={{ borderColor: 'divider', borderTopWidth: '1px' }} flexDir="column" rowGap={2}>
              {content}
            </Flex>
          );
        })}
