...
const TokenInstanceDetails = ({ data, scrollRef, isLoading }: Props) => {
  ...
  return (
    <>
      <Flex alignItems="flex-start" flexDir={{ base: 'column-reverse', lg: 'row' }} columnGap={6} rowGap={6}>
        ...
        >
          {data.is_unique && data.owner && (
            <DetailsInfoItem title="Owner" hint="Current owner of this certificate instance" isLoading={isLoading}>
              <AddressEntity address={data.owner} isLoading={isLoading} />
            </DetailsInfoItem>
          )}
          <TokenInstanceCreatorAddress hash={isLoading ? '' : data.token.address} />
          <DetailsInfoItem title="Certificate ID" hint="This certificate instance unique certificate ID" isLoading={isLoading}>
      ...
      <Grid mt={5} columnGap={8} rowGap={{ base: 1, lg: 3 }} templateColumns={{ base: 'minmax(0, 1fr)', lg: '200px minmax(0, 1fr)' }} overflow="hidden">
