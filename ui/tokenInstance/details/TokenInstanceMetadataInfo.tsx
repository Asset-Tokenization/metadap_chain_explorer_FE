...
const Item = ({ data, isLoading }: ItemProps) => {
  ...
  const value = (() => {
    if (data.value_type === 'URL') {
      return (
        <LinkExternal whiteSpace="nowrap" display="inline-flex" alignItems="center" w="100%" overflow="hidden" href={data.value}>
  ...
  return (
    <GridItem bgColor={attributeBgColor} borderRadius="md" px={4} py={2} display="flex" flexDir="column" alignItems="flex-start">
...
const TokenInstanceMetadataInfo = ({ data, isLoading }: Props) => {
  ...
  return (
    <>
      ...
      {metadata?.name && (
        <DetailsInfoItem title="Name" hint="NFT name" whiteSpace="normal" wordBreak="break-word" isLoading={isLoading}>
          <Skeleton isLoaded={!isLoading}>{metadata.name}</Skeleton>
        </DetailsInfoItem>
      )}
      {metadata?.description && (
        <DetailsInfoItem title="Description" hint="NFT description" whiteSpace="normal" wordBreak="break-word" isLoading={isLoading}>
          <Skeleton isLoaded={!isLoading}>{metadata.description}</Skeleton>
        </DetailsInfoItem>
      )}
      {metadata?.attributes && (
        <DetailsInfoItem title="Attributes" hint="NFT attributes" whiteSpace="normal" isLoading={isLoading}>
