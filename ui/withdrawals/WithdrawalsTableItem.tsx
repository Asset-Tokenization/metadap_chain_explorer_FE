...
const WithdrawalsTableItem = ({ item, view, isLoading }: Props) => {
  return (
    <Tr>
      ...
      {view !== 'block' && (
        <Td verticalAlign="middle">
          <BlockEntity number={item.block_number} isLoading={isLoading} fontSize="sm" lineHeight={5} />
        </Td>
      )}
      {view !== 'address' && (
        <Td verticalAlign="middle">
          <AddressEntity address={item.receiver} isLoading={isLoading} truncation="constant" />
        </Td>
      )}
      {view !== 'block' && (
