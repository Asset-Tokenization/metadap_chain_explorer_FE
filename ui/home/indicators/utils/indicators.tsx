...
const dailyTxsIndicator: TChainIndicator<'homepage_chart_txs'> = {
  ...
  api: {
    ...
    dataFn: (response) => [
      {
        items: response.chart_data.map((item) => ({ date: new Date(item.date), value: item.tx_count })).sort(sortByDateDesc),
        name: 'Tx/day',
...
const coinPriceIndicator: TChainIndicator<'homepage_chart_market'> = {
  ...
  hint: `${config.chain.governanceToken.symbol || config.chain.currency.symbol} certificate daily price in USD.`,
  api: {
...
const INDICATORS = [dailyTxsIndicator, coinPriceIndicator, marketPriceIndicator];
