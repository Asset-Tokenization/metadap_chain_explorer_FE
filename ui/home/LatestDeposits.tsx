...
const LatestDeposits = () => {
  ...
  const handleNewDepositMessage: SocketMessage.NewDeposits['handler'] = React.useCallback(
    (payload) => {
      ...
      <>
        ...
        <Box mb={{ base: 3, lg: 4 }}>
          {data.slice(0, itemsCount).map((item, index) => (
            <LatestDepositsItem key={item.l2_tx_hash + (isPlaceholderData ? index : '')} item={item} isLoading={isPlaceholderData} />
          ))}
        </Box>
        <Flex justifyContent="center">
          <LinkInternal fontSize="sm" href={depositsUrl}>
            View all deposits
          </LinkInternal>
        </Flex>
      </>
