    ...
    >
      <Flex overflow="hidden" w="100%">
        ...
        <Box ml={3} w="calc(100% - 40px)">
          ...
          <Flex mt={2} alignItems="center">
            ...
            {tx.timestamp && (
              <Skeleton isLoaded={!isLoading} color="text_secondary" fontWeight="400" fontSize="sm" flexShrink={0} ml={2}>
                <span>{timeAgo}</span>
              </Skeleton>
            )}
          </Flex>
      ...
      <Grid alignItems="center" alignSelf="flex-start" templateColumns="24px auto">
        ...
        <Box overflow="hidden" ml={1}>
          <AddressEntity isLoading={isLoading} address={tx.from} fontSize="sm" lineHeight={6} fontWeight="500" mb={2} />
          {dataTo && <AddressEntity isLoading={isLoading} address={dataTo} fontSize="sm" lineHeight={6} fontWeight="500" />}
        </Box>
      </Grid>
      <Box>
        ...
        {!config.UI.views.tx.hiddenFields?.tx_fee && (
          <Skeleton isLoaded={!isLoading}>
            ...
            <Text as="span" variant="secondary">
              {getValueWithUnit(tx.fee.value).dp(5).toFormat()}
            </Text>
          </Skeleton>
        )}
      </Box>
