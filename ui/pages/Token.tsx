...
const TokenPageContent = () => {
  ...
  const verifiedInfoQuery = useApiQuery('token_verified_info', {
    ...
    queryOptions: {
      enabled: Boolean(tokenQuery.data) && config.features.verifiedTokens.isEnabled,
    },
  });
  ...
  const tabs: Array<RoutedTab> = [
    tokenQuery.data?.type === 'ERC-1155' || tokenQuery.data?.type === 'ERC-721'
      ? {
          id: 'inventory',
          title: 'Inventory',
          component: <TokenInventory inventoryQuery={inventoryQuery} />,
        }
      : undefined,
    {
      id: 'token_transfers',
    ...
    },
    {
      id: 'holders',
      title: 'Holders',
      component: <TokenHolders token={tokenQuery.data} holdersQuery={holdersQuery} />,
    },
  ...
  const backLink = React.useMemo(() => {
    ...
    return {
      label: 'Back to certificates list',
  ...
  const titleSecondRow = (
    <Flex alignItems="center" w="100%" minW={0} columnGap={2} rowGap={2} flexWrap={{ base: 'wrap', lg: 'nowrap' }}>
      <AddressEntity address={{ ...contractQuery.data, name: '' }} isLoading={isLoading} fontFamily="heading" fontSize="lg" fontWeight={500} />
  ...
  return (
    <>
      <TextAd mb={6} />
      <PageTitle
        title={`${tokenQuery.data?.name || 'Unnamed certificate'}${tokenSymbolText}`}
        isLoading={isLoading}
        backLink={backLink}
