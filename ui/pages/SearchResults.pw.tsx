  ...
  extendedTest('+@mobile +@dark-mode', async ({ mount, page }) => {
    ...
    await page.route(buildApiUrl('search') + '?q=o', (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
          items: [searchMock.token1, searchMock.token2, searchMock.contract1, searchMock.label1],
        }),
      }),
    );
...
test('search by address hash +@mobile', async ({ mount, page }) => {
  ...
  await page.route(buildApiUrl('search') + `?q=${searchMock.address1.address}`, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify({
...
test('search by block number +@mobile', async ({ mount, page }) => {
  ...
  await page.route(buildApiUrl('search') + `?q=${searchMock.block1.block_number}`, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify({
        items: [searchMock.block1, searchMock.block2],
      }),
    }),
  );
...
test('search by block hash +@mobile', async ({ mount, page }) => {
  ...
  await page.route(buildApiUrl('search') + `?q=${searchMock.block1.block_hash}`, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify({
...
test('search by tx hash +@mobile', async ({ mount, page }) => {
  ...
  await page.route(buildApiUrl('search') + `?q=${searchMock.tx1.tx_hash}`, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify({
