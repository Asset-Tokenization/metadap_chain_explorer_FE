...
const TokenInstanceContent = () => {
  ...
  const holdersQuery = useQueryWithPages({
    ...
    options: {
      ...
      placeholderData: generateListStub<'token_instance_holders'>(
        tokenInstanceQuery.data?.token.type === 'ERC-1155' ? tokenStubs.TOKEN_HOLDER_ERC_1155 : tokenStubs.TOKEN_HOLDER_ERC_20,
        10,
        { next_page_params: null },
      ),
    },
      ...
      metadata.update(
        {
          ...
          query: {
            hash: tokenInstanceQuery.data.token.address,
  ...
  const backLink = React.useMemo(() => {
    ...
    return {
      label: 'Back to certificate page',
  ...
  const tabs: Array<RoutedTab> = [
    ...
    shouldFetchHolders
      ? {
          id: 'holders',
          title: 'Holders',
          component: <TokenHolders holdersQuery={holdersQuery} token={tokenInstanceQuery.data?.token} />,
        }
      : undefined,
    {
      id: 'metadata',
      title: 'Metadata',
      component: <TokenInstanceMetadata data={tokenInstanceQuery.data?.metadata} isPlaceholderData={tokenInstanceQuery.isPlaceholderData} />,
    },
  ].filter(Boolean);

  if (tokenInstanceQuery.isError) {
    throw Error('Certificate instance fetch failed', {
      cause: tokenInstanceQuery.error,
    });
  }
  ...
  return (
    <>
      <TextAd mb={6} />
      <PageTitle title={`ID ${tokenInstanceQuery.data?.id}`} backLink={backLink} contentAfter={tokenTag} secondRow={titleSecondRow} isLoading={isLoading} />
