  ...
  const content = (() => {
    switch (data.token.type) {
      case 'ERC-20': {
        ...
        return (
          <Flex flexWrap="wrap" columnGap={2} rowGap={2}>
            ...
            <span>{valueStr}</span>
            <TokenEntity token={{ ...data.token, name: data.token.symbol || data.token.name }} noCopy noSymbol w="auto" />
      ...
      case 'ERC-1155': {
        ...
        return <NftTokenTransferSnippet key={total.token_id} token={data.token} tokenId={total.token_id} value={total.value} />;
      }
    }
  })();

  return (
    <Flex alignItems="flex-start" flexWrap={{ base: 'wrap', lg: 'nowrap' }} columnGap={2} rowGap={3} flexDir="row" w="100%">
