import { Grid, GridItem, Text, Box, Icon as ChakraIcon, Link, Spinner, Flex, Tooltip, chakra, useColorModeValue, Skeleton, Alert } from '@chakra-ui/react';
...
const TxDetails = () => {
  ...
  const addressFromTags = [...(data.from.private_tags || []), ...(data.from.public_tags || []), ...(data.from.watchlist_names || [])].map((tag) => (
    <Tag key={tag.label}>{tag.display_name}</Tag>
  ));

  const toAddress = data.to ? data.to : data.created_contract;
  const addressToTags = [...(toAddress?.private_tags || []), ...(toAddress?.public_tags || []), ...(toAddress?.watchlist_names || [])].map((tag) => (
    <Tag key={tag.label}>{tag.display_name}</Tag>
  ));
  ...
  return (
    <>
      ...
      >
        ...
        <DetailsInfoItem title="Block" hint="Block number containing the transaction" isLoading={isPlaceholderData}>
          {data.block === null ? <Text>Pending</Text> : <BlockEntity isLoading={isPlaceholderData} number={data.block} noIcon />}
        ...
        {data.execution_node && (
          <DetailsInfoItem title="Computor" hint="Node that carried out the confidential computation" isLoading={isPlaceholderData}>
            <AddressEntity
              address={data.execution_node}
              href={route({
                pathname: '/txs/computor/[hash]',
        ...
        <DetailsInfoItem title="From" hint="Address (external or contract) sending the transaction" isLoading={isPlaceholderData} columnGap={3}>
            ...
            <>
              {data.to && data.to.hash ? (
                <Flex flexWrap="nowrap" alignItems="center" maxW="100%">
                  <AddressEntity address={toAddress} isLoading={isPlaceholderData} />
                ...
                <Flex width="100%" whiteSpace="pre" alignItems="center" flexShrink={0}>
                  <span>[Contract </span>
                  <AddressEntity address={toAddress} isLoading={isPlaceholderData} noIcon />
                  <span>created]</span>
                  {executionSuccessBadge}
                  {executionFailedBadge}
                </Flex>
        ...
        {!config.UI.views.tx.hiddenFields?.value && (
          <DetailsInfoItem title="Value" hint="Value sent in the native certificate (and USD) if applicable" isLoading={isPlaceholderData}>
            <CurrencyValue
              value={data.value}
        ...
        {!config.UI.views.tx.hiddenFields?.tx_fee && (
          <DetailsInfoItem title="Transaction fee" hint="Total transaction fee" isLoading={isPlaceholderData}>
            <CurrencyValue
        ...
        <DetailsInfoItem title="Gas usage & limit by txn" hint="Actual gas amount used by the transaction" isLoading={isPlaceholderData}>
        ...
        {!config.UI.views.tx.hiddenFields?.gas_fees && (data.base_fee_per_gas || data.max_fee_per_gas || data.max_priority_fee_per_gas) && (
          <DetailsInfoItem
            title="Gas fees (Gwei)"
        ...
        {data.tx_burnt_fee && !config.UI.views.tx.hiddenFields?.burnt_fees && !config.features.rollup.isEnabled && (
          ...
          >
            ...
            <CurrencyValue value={String(data.tx_burnt_fee)} currency={config.chain.currency.symbol} exchangeRate={data.exchange_rate} flexWrap="wrap" ml={2} />
          </DetailsInfoItem>
        )}
        {config.features.rollup.isEnabled && (
          <>
            {data.l1_gas_used && (
              <DetailsInfoItem title="L1 gas used by txn" hint="L1 gas used by transaction" isLoading={isPlaceholderData}>
                <Text>{BigNumber(data.l1_gas_used).toFormat()}</Text>
              </DetailsInfoItem>
            )}
            {data.l1_gas_price && (
              <DetailsInfoItem title="L1 gas price" hint="L1 gas price" isLoading={isPlaceholderData}>
                <Text mr={1}>
                  {BigNumber(data.l1_gas_price).dividedBy(WEI).toFixed()} {config.chain.currency.symbol}
                </Text>
            ...
            {data.l1_fee && (
              <DetailsInfoItem
                ...
                isLoading={isPlaceholderData}
              >
                <CurrencyValue value={data.l1_fee} currency={config.chain.currency.symbol} exchangeRate={data.exchange_rate} flexWrap="wrap" />
              </DetailsInfoItem>
            )}
            {data.l1_fee_scalar && (
              <DetailsInfoItem
        ...
        <GridItem colSpan={{ base: undefined, lg: 2 }}>
          <Element name="TxDetails__cutLink">
            <Skeleton isLoaded={!isPlaceholderData} mt={6} display="inline-block">
              <Link display="inline-block" fontSize="sm" textDecorationLine="underline" textDecorationStyle="dashed" onClick={handleCutClick}>
        ...
        {isExpanded && (
          <>
            ...
            <DetailsInfoItem title="Raw input" hint="Binary data included with the transaction. See logs tab for additional info">
