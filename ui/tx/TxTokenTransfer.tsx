...
const TxTokenTransfer = () => {
  ...
  const tokenTransferQuery = useQueryWithPages({
    ...
    options: {
      enabled:
        !txsInfo.isPlaceholderData &&
        Boolean(txsInfo.data?.status && txsInfo.data?.hash),
  ...
  if (
    ...
    return txsInfo.socketStatus ? (
      <TxSocketAlert status={txsInfo.socketStatus} />
    ) : (
      <TxPendingAlert />
    );
  }
  ...
  const content = tokenTransferQuery.data?.items ? (
    <>
      <Hide below="lg" ssr={false}>
        <TokenTransferTable
          data={tokenTransferQuery.data?.items}
  ...
  return (
    <DataListDisplay
      ...
      emptyText="There are no certificate transfers."
      filterProps={{
        emptyFilteredText: `Couldn${apos}t find any certificate transfer that matches your query.`,
