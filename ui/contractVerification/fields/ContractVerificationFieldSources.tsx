...
const ContractVerificationFieldSources = ({ fileTypes, multiple, required, title, hint, name = 'sources' }: Props) => {
  ...
  const renderFiles = React.useCallback(
    (files: Array<File>) => {
      ...
      return (
        <Box display="grid" gridTemplateColumns={{ base: 'minmax(0, 1fr)', lg: 'minmax(0, 1fr) minmax(0, 1fr)' }} columnGap={3} rowGap={3} w="100%">
      ...
      return (
        <>
          <FileInput<FormFields, typeof name> accept={fileTypes.join(',')} multiple={multiple} field={field}>
            {({ onChange }) => (
              <Flex flexDir="column" alignItems="flex-start" rowGap={2} w="100%">
  ...
  const validateFileType = React.useCallback(
    async (value: FieldPathValue<FormFields, typeof name>): Promise<ValidateResult> => {
  ...
  return (
    <ContractVerificationFormRow>
      <Controller name={name} control={control} render={renderControl} rules={rules} />
