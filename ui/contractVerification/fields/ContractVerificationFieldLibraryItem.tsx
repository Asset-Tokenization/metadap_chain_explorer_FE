...
const ContractVerificationFieldLibraryItem = ({ control, index, fieldsLength, onAddFieldClick, onRemoveFieldClick, error, isDisabled }: Props) => {
  ...
  const renderNameControl = React.useCallback(
    ({ field }: { field: ControllerRenderProps<FormFields, `libraries.${number}.name`> }) => {
      return (
        <FormControl variant="floating" id={field.name} isRequired size={{ base: 'md', lg: 'lg' }}>
          <Input {...field} required isInvalid={Boolean(error?.name)} isDisabled={isDisabled} maxLength={255} autoComplete="off" />
  ...
  const renderAddressControl = React.useCallback(
    ({ field }: { field: ControllerRenderProps<FormFields, `libraries.${number}.address`> }) => {
      return (
        <FormControl variant="floating" id={field.name} isRequired size={{ base: 'md', lg: 'lg' }}>
          <Input {...field} isInvalid={Boolean(error?.address)} isDisabled={isDisabled} required autoComplete="off" />
  ...
  return (
    <>
      ...
      <ContractVerificationFormRow>
        <Controller name={`libraries.${index}.name`} control={control} render={renderNameControl} rules={{ required: true }} />
        {index === 0 ? <>A library name called in the .sol file. Multiple libraries (up to 10) may be added for each contract.</> : null}
      </ContractVerificationFormRow>
      <ContractVerificationFormRow>
        <Controller name={`libraries.${index}.address`} control={control} render={renderAddressControl} rules={{ required: true, pattern: ADDRESS_REGEXP }} />
        {index === 0 ? <>The 0x library address. This can be found in the generated json file or Truffle output (if using truffle).</> : null}
      </ContractVerificationFormRow>
    </>
  );
};
