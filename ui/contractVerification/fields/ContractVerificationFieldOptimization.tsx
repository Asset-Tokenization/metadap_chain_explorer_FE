...
const ContractVerificationFieldOptimization = () => {
  ...
  const renderCheckboxControl = React.useCallback(
    ({ field }: { field: ControllerRenderProps<FormFields, 'is_optimization_enabled'> }) => (
      <Flex flexShrink={0}>
  ...
  const renderInputControl = React.useCallback(
    ({ field }: { field: ControllerRenderProps<FormFields, 'optimization_runs'> }) => {
      return (
        <Input
          {...field}
          required
  ...
  return (
    <ContractVerificationFormRow>
      <Flex columnGap={5} h={{ base: 'auto', lg: '32px' }}>
        <Controller name="is_optimization_enabled" control={control} render={renderCheckboxControl} />
        {isEnabled && <Controller name="optimization_runs" control={control} render={renderInputControl} rules={{ required: true }} />}
      </Flex>
    </ContractVerificationFormRow>
  );
};
