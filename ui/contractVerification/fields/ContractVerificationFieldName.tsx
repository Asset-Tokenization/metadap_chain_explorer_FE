...
const ContractVerificationFieldName = ({ hint, isReadOnly }: Props) => {
    ...
    ({ field }: { field: ControllerRenderProps<FormFields, 'name'> }) => {
      ...
      return (
        <FormControl variant="floating" id={field.name} isRequired size={{ base: 'md', lg: 'lg' }}>
          <Input {...field} required isInvalid={Boolean(error)} maxLength={255} isDisabled={formState.isSubmitting || isReadOnly} autoComplete="off" />
  ...
  return (
    <ContractVerificationFormRow>
      <Controller name="name" control={control} render={renderControl} rules={{ required: true }} />
      {hint ? (
        <span>{hint}</span>
      ) : (
        <>
          ...
          <span>
            . <chakra.span fontWeight={600}>MyContract</chakra.span> is the contract name.
          </span>
        </>
      )}
    </ContractVerificationFormRow>
