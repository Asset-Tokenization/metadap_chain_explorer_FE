...
const ContractVerificationFieldCode = ({ isVyper }: Props) => {
    ...
    ({ field }: { field: ControllerRenderProps<FormFields, 'code'> }) => {
      ...
      return (
        <FormControl variant="floating" id={field.name} isRequired size={{ base: 'md', lg: 'lg' }}>
          <Textarea {...field} isInvalid={Boolean(error)} isDisabled={formState.isSubmitting} required />
  ...
  return (
    <ContractVerificationFormRow>
      <Controller name="code" control={control} render={renderControl} rules={{ required: true }} />
      {isVyper ? null : (
        <>
          ...
          <Link href="https://github.com/poanetwork/solidity-flattener" target="_blank">
            POA solidity flattener
          </Link>
          <span> or the </span>
          <Link href="https://www.npmjs.com/package/truffle-flattener" target="_blank">
            Truffle flattener
          </Link>
        </>
      )}
