...
const BlockDetails = ({ query }: Props) => {
  ...
  return (
    ...
    >
      ...
      <DetailsInfoItem title="Transactions" hint="The number of transactions in the block" isLoading={isPlaceholderData}>
      ...
      {config.features.beaconChain.isEnabled && Boolean(data.withdrawals_count) && (
        <DetailsInfoItem title="Withdrawals" hint="The number of beacon withdrawals in the block" isLoading={isPlaceholderData}>
          <Skeleton isLoaded={!isPlaceholderData}>
            <LinkInternal
              href={route({
                pathname: '/block/[height_or_hash]',
      ...
      {!config.features.rollup.isEnabled && !totalReward.isEqualTo(ZERO) && !config.UI.views.block.hiddenFields?.total_reward && (
        <DetailsInfoItem
          title="Block reward"
          hint={`For each block, the ${validatorTitle} is rewarded with a finite amount of ${config.chain.currency.symbol || 'native certificate'} 
      ...
      <DetailsInfoItem title="Gas limit" hint="Total gas limit provided by all transactions in the block" isLoading={isPlaceholderData}>
      ...
      {data.minimum_gas_price && (
        <DetailsInfoItem
          ...
          isLoading={isPlaceholderData}
        >
          <Skeleton isLoaded={!isPlaceholderData}>{BigNumber(data.minimum_gas_price).dividedBy(GWEI).toFormat()} Gwei</Skeleton>
        </DetailsInfoItem>
      )}
      {data.base_fee_per_gas && (
        ...
        >
          {isPlaceholderData ? (
            ...
            <>
              <Text>
                {BigNumber(data.base_fee_per_gas).dividedBy(WEI).toFixed()} {config.chain.currency.symbol}{' '}
              </Text>
      ...
      {!config.UI.views.block.hiddenFields?.burnt_fees && (
        <DetailsInfoItem
          title="Burnt fees"
          hint={`Amount of ${config.chain.currency.symbol || 'native certificate'} burned from transactions included in the block.
      ...
      {data.priority_fee !== null && BigNumber(data.priority_fee).gt(ZERO) && (
        <DetailsInfoItem title="Priority fee / Tip" hint="User-defined tips sent to validator for transaction priority/inclusion" isLoading={isPlaceholderData}>
          <Skeleton isLoaded={!isPlaceholderData}>
      ...
      {isExpanded && !isPlaceholderData && (
        <>
          ...
          {data.bitcoin_merged_mining_header && (
            <DetailsInfoItem title="Bitcoin merged mining header" hint="Merged-mining field: Bitcoin header" flexWrap="nowrap" alignSelf="flex-start">
          ...
          {data.bitcoin_merged_mining_coinbase_transaction && (
            <DetailsInfoItem title="Bitcoin merged mining coinbase transaction" hint="Merged-mining field: Coinbase transaction">
              <RawDataSnippet data={data.bitcoin_merged_mining_coinbase_transaction} isLoading={isPlaceholderData} showCopy={false} textareaMaxHeight="100px" />
            </DetailsInfoItem>
          )}
          {data.bitcoin_merged_mining_merkle_proof && (
            <DetailsInfoItem title="Bitcoin merged mining Merkle proof" hint="Merged-mining field: Merkle proof">
              <RawDataSnippet data={data.bitcoin_merged_mining_merkle_proof} isLoading={isPlaceholderData} showCopy={false} textareaMaxHeight="100px" />
            </DetailsInfoItem>
          )}
          {data.hash_for_merged_mining && (
            <DetailsInfoItem title="Hash for merged mining" hint="Merged-mining field: Rootstock block header hash" flexWrap="nowrap" alignSelf="flex-start">
              <Box whiteSpace="nowrap" overflow="hidden">
          ...
          <DetailsInfoItem title="Difficulty" hint={`Block difficulty for ${validatorTitle}, used to calibrate block generation time`}>
            <Box whiteSpace="nowrap" overflow="hidden">
          ...
          {data.height > 0 && (
            <DetailsInfoItem title="Parent hash" hint="The hash of the block from which this block was generated" flexWrap="nowrap">
              <LinkInternal
                href={route({
                  pathname: '/block/[height_or_hash]',
          ...
          {!config.UI.views.block.hiddenFields?.nonce && (
            <DetailsInfoItem title="Nonce" hint="Block nonce is a value used during mining to demonstrate proof of work for a block">
              {data.nonce}
            </DetailsInfoItem>
          )}
        </>
