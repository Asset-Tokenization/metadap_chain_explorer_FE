...
const AddressForm: React.FC<Props> = ({ data, onSuccess, setAlertVisible, isAdd }) => {
  ...
  const renderAddressInput = useCallback(
    ({ field }: { field: ControllerRenderProps<Inputs, 'address'> }) => {
      return <AddressInput<Inputs, 'address'> field={field} backgroundColor={formBackgroundColor} error={errors.address} />;
    },
  ...
  const renderCheckbox = useCallback(
    (text: string) =>
      ({ field }: { field: ControllerRenderProps<Inputs, Checkboxes> }) => <CheckboxInput<Inputs, Checkboxes> text={text} field={field} />,
    [],
  );
  ...
  return (
    <form noValidate onSubmit={handleSubmit(onSubmit)}>
      ...
      <Text variant="secondary" fontSize="sm" marginBottom={{ base: '10px', lg: 5 }}>
        Notification methods
      </Text>
      <Controller name={'notification' as Checkboxes} control={control} render={renderCheckbox('Email notifications')} />
      <Box marginTop={8}>
        <Button size="lg" type="submit" isLoading={pending} isDisabled={!isDirty}>
