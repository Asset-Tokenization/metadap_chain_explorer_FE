...
const ContractWrite = ({ addressHash, isProxy, isCustomAbi }: Props) => {
  ...
  const handleMethodFormSubmit = React.useCallback(
    async (item: SmartContractWriteMethod, args: Array<string | Array<unknown>>) => {
      if (!isConnected) {
  ...
  const renderItemContent = React.useCallback(
    (item: SmartContractWriteMethod, index: number, id: number) => {
      return <ContractMethodCallable key={id + '_' + index} data={item} onSubmit={handleMethodFormSubmit} resultComponent={ContractWriteResult} isWrite />;
    },
