...
test("filter", async ({ mount, page }) => {
  ...
  await page.getByRole("button", { name: /select/i }).click();
  await page.getByPlaceholder("Search by certificate name").type("c");
...
test.describe("socket", () => {
  ...
  testWithSocket(
    "new item after token balance update",
    async ({ page, mount, createSocket }) => {
      await mount(
        <TestApp withSocket>
          <MockAddressPage>
            <Flex>
      ...
      await page.route(
        ...
        async (route) =>
          route.fulfill({
            status: 200,
            body: JSON.stringify({
              items: [...tokensMock.erc20List.items, tokensMock.erc20d],
            }),
          }),
        { times: 1 }
      );
  ...
  testWithSocket(
    "new item after coin balance update",
    async ({ page, mount, createSocket }) => {
      await mount(
        <TestApp withSocket>
          <MockAddressPage>
