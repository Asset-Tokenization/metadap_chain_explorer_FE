...
const TokenSelect = ({ onClick }: Props) => {
  ...
  const { data, isError, isLoading, refetch } = useFetchTokens({
  ...
  const handleTokenBalanceMessage: SocketMessage.AddressTokenBalance['handler'] = React.useCallback(
    (payload) => {
  ...
  const handleCoinBalanceMessage: SocketMessage.AddressCoinBalance['handler'] = React.useCallback(
    (payload) => {
  ...
  return (
    <Flex columnGap={3} mt={{ base: '6px', lg: 0 }}>
      {isMobile ? <TokenSelectMobile data={data} isLoading={tokensIsFetching === 1} /> : <TokenSelectDesktop data={data} isLoading={tokensIsFetching === 1} />}
      <Tooltip label="Show all certificates">
        <Box>
          <NextLink
            href={{
