...
const TxnBatchesTableItem = ({ item, isLoading }: Props) => {
  ...
  return (
    <Tr>
      ...
      <Td>
        <BlockEntityL1 isLoading={isLoading} number={item.epoch_number} fontSize="sm" lineHeight={5} fontWeight={600} py="2px" noIcon />
      </Td>
      <Td pr={12}>
        <VStack spacing={3} alignItems="flex-start">
          {item.l1_tx_hashes.map((hash) => (
            <TxEntityL1 key={hash} isLoading={isLoading} hash={hash} fontSize="sm" lineHeight={5} maxW="100%" />
          ))}
        </VStack>
      </Td>
      <Td>
