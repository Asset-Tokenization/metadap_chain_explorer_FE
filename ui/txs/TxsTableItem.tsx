import { Tr, Td, VStack, Show, Hide, Flex, Skeleton, Box } from '@chakra-ui/react';
...
const TxsTableItem = ({ tx, showBlockInfo, currentAddress, enableTimeIncrement, isLoading }: Props) => {
  ...
  const addressFrom = <AddressEntity address={tx.from} isLoading={isLoading} noCopy={isOut} noLink={isOut} truncation="constant" w="100%" py="2px" />;

  const addressTo = dataTo ? <AddressEntity address={dataTo} isLoading={isLoading} truncation="constant" noCopy={isIn} noLink={isIn} w="100%" py="2px" /> : '-';
    ...
    >
      ...
      <Td pr={4}>
        <VStack alignItems="start" lineHeight="24px">
          <TxEntity hash={tx.hash} isLoading={isLoading} fontWeight={700} noIcon maxW="100%" />
          {tx.timestamp && (
      ...
      {showBlockInfo && <Td>{tx.block && <BlockEntity isLoading={isLoading} number={tx.block} noIcon fontSize="sm" lineHeight={6} fontWeight={500} />}</Td>}
        ...
        <Td colSpan={3}>
          <Flex alignItems="center">
            {isIn || isOut ? (
              ...
              <Icon as={rightArrowIcon} boxSize={6} color="gray.500" transform="rotate(90deg)" isLoading={isLoading} />
            )}
