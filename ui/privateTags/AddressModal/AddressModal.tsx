...
const AddressModal: React.FC<Props> = ({ isOpen, onClose, onSuccess, data, pageType }) => {
  ...
  React.useEffect(() => {
    isOpen && !data?.id && mixpanel.logEvent(mixpanel.EventTypes.PRIVATE_TAG, { Action: 'Form opened', 'Page type': pageType, 'Tag type': 'Address' });
  }, [data?.id, isOpen, pageType]);

  const handleSuccess = React.useCallback(() => {
    if (!data?.id) {
      mixpanel.logEvent(mixpanel.EventTypes.PRIVATE_TAG, { Action: 'Submit', 'Page type': pageType, 'Tag type': 'Address' });
    }
    return onSuccess();
