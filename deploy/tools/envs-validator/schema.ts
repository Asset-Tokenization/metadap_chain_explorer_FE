...
const marketplaceSchema = yup.object().shape({
  NEXT_PUBLIC_MARKETPLACE_CONFIG_URL: yup.array().json().of(marketplaceAppSchema),
  NEXT_PUBLIC_MARKETPLACE_SUBMIT_FORM: yup.string().when('NEXT_PUBLIC_MARKETPLACE_CONFIG_URL', {
...
const beaconChainSchema = yup.object().shape({
  ...
  NEXT_PUBLIC_BEACON_CHAIN_CURRENCY_SYMBOL: yup.string().when('NEXT_PUBLIC_HAS_BEACON_CHAIN', {
    ...
    otherwise: (schema) => schema.max(-1, 'NEXT_PUBLIC_BEACON_CHAIN_CURRENCY_SYMBOL cannot not be used if NEXT_PUBLIC_HAS_BEACON_CHAIN is not set to "true"'),
  }),
});

const rollupSchema = yup.object().shape({
  NEXT_PUBLIC_IS_L2_NETWORK: yup.boolean(),
  NEXT_PUBLIC_L1_BASE_URL: yup.string().when('NEXT_PUBLIC_IS_L2_NETWORK', {
    is: (value: boolean) => value,
    then: (schema) => schema.test(urlTest).required(),
    otherwise: (schema) => schema.max(-1, 'NEXT_PUBLIC_L1_BASE_URL cannot not be used if NEXT_PUBLIC_IS_L2_NETWORK is not set to "true"'),
  }),
  NEXT_PUBLIC_L2_WITHDRAWAL_URL: yup.string().when('NEXT_PUBLIC_IS_L2_NETWORK', {
...
const schema = yup
  ...
  .shape({
    ...
    //     d. views
    NEXT_PUBLIC_VIEWS_BLOCK_HIDDEN_FIELDS: yup.array().transform(replaceQuotes).json().of(yup.string<BlockFieldId>().oneOf(BLOCK_FIELDS_IDS)),
    NEXT_PUBLIC_VIEWS_ADDRESS_IDENTICON_TYPE: yup.string().oneOf(IDENTICON_TYPES),
    NEXT_PUBLIC_VIEWS_ADDRESS_HIDDEN_VIEWS: yup.array().transform(replaceQuotes).json().of(yup.string<AddressViewId>().oneOf(ADDRESS_VIEWS_IDS)),
    NEXT_PUBLIC_VIEWS_TX_HIDDEN_FIELDS: yup.array().transform(replaceQuotes).json().of(yup.string<TxFieldsId>().oneOf(TX_FIELDS_IDS)),
    NEXT_PUBLIC_VIEWS_TX_ADDITIONAL_FIELDS: yup.array().transform(replaceQuotes).json().of(yup.string<TxAdditionalFieldsId>().oneOf(TX_ADDITIONAL_FIELDS_IDS)),
