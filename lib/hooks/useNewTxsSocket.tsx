...
function getSocketParams(router: NextRouter) {
  if (router.pathname === '/txs' && (router.query.tab === 'validated' || router.query.tab === undefined) && !router.query.block_number && !router.query.page) {
  ...
  if (router.pathname === '/txs' && router.query.tab === 'pending' && !router.query.block_number && !router.query.page) {
...
export default function useNewTxsSocket() {
  ...
  const handleNewTxMessage = React.useCallback(
    (response: { transaction: number } | { pending_transaction: number } | unknown) => {
